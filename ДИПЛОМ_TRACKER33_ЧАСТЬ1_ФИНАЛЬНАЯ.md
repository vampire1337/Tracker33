...
Итоговая архитектура системы построена на следующем стеке:

```
Frontend (Desktop): PyQt5 + Python
├── Мониторинг активности: psutil, pynput, pygetwindow
├── Windows API: win32gui, win32process
├── HTTP клиент: requests
└── Конфигурация: configparser, json

Backend: Django + Django REST Framework
├── База данных: SQLite (с возможностью миграции на PostgreSQL)
├── Аутентификация: Token Authentication
├── Кэширование: django-cache
...
Такой выбор оказался оправданным в процессе разработки. Python позволил быстро реализовать сложную логику мониторинга, а Django предоставил готовые решения для многих типовых задач веб-разработки.
...
```mermaid
graph TB
    A[Desktop Client] --> B[Django Server]
    B --> C[SQLite Database]
    B --> D[REST API]
    D --> E[Web Interface]
...
"Desktop Application" -> "Django Server" : HTTP/REST
"Django Server" -> "Database Layer" : ORM
@enduml
```

Система спроектирована по принципу разделения ответственности:
- **Клиентское приложение** отвечает за сбор данных и предоставление интерфейса
- **Серверная часть** обрабатывает данные и предоставляет API
- **База данных** хранит информацию и обеспечивает целостность
