...
Для установки TimeTracker на Windows:

1. Скачайте последнюю версию приложения с [сайта проекта](https://github.com/yourusername/Tracker33/releases)
2. Распакуйте архив в любую директорию
3. Запустите `install.bat` с правами администратора
4. Следуйте инструкциям на экране

После установки приложение будет доступно в меню "Пуск" и автоматически запускаться при старте системы.

### Linux

Для установки TimeTracker на Ubuntu Linux и другие Debian-подобные дистрибутивы:
...
После установки приложение будет доступно в меню приложений и автоматически запускаться при входе в систему.

### Установка из исходного кода

Если вы хотите установить приложение из исходного кода:

1. Клонируйте репозиторий:
```bash
git clone https://github.com/yourusername/Tracker33.git
cd Tracker33
```
...
После установки приложение запустится автоматически и будет доступно в области уведомлений (системном трее).

Вы также можете запустить приложение:
- В Windows: через меню "Пуск" или выполнив `TimeTracker.exe` из директории установки
- В Linux: через меню приложений или выполнив команду `TimeTracker` в терминале

## Настройка

При первом запуске вам будет предложено ввести учетные данные для подключения к серверу:

1. Откройте главное окно приложения, кликнув по иконке в системном трее
2. Перейдите в раздел "Настройки"
3. Введите URL сервера и свои учетные данные
4. Нажмите "Сохранить"
...
- Поведение в режиме простоя (когда пользователь неактивен)

## Удаление приложения

### Windows
1. Перейдите в "Панель управления" > "Программы и компоненты"
2. Найдите TimeTracker в списке и выберите "Удалить"
3. Или запустите скрипт `cleanup.py` с параметром `--uninstall`:
```
python desktop_app/cleanup.py --uninstall
```

### Linux
1. Запустите скрипт удаления:
```bash
python ~/.local/share/TimeTracker/cleanup.py --uninstall
```
